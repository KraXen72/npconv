      ...
      <div class="route-grid">
        <article class="route-card video-route" classList={{ selected: isVideo() }}>
          <button class="route-main" type="button" onClick={() => select(props.mode() === 'convert' ? 'convert' : 'merge')} aria-pressed={isVideo()}>
            ...
            <span class="route-apps"><AppNode icon={<Play size={18} />} name="NewPipe" /><span class="route-arrow"><ArrowLeftRight size={20} /></span><AppNode icon={<DatabaseBackup size={18} />} name="LibreTube" /></span>
          </button>
        ...
        <button class="route-card route-main" classList={{ selected: props.mode() === 'stt' }} type="button" onClick={() => select('stt')} aria-pressed={props.mode() === 'stt'}>
          ...
          <span class="route-apps"><AppNode icon={<TimerReset size={18} />} name="Simple Time Tracker" /><span class="route-arrow"><ArrowRight size={20} /></span><AppNode icon={<History size={18} />} name="Loop Habits" /></span>
        </button>
        ...
        <button class="route-card route-main" classList={{ selected: props.mode() === 'timejot' }} type="button" onClick={() => select('timejot')} aria-pressed={props.mode() === 'timejot'}>
          ...
          <span class="route-apps"><AppNode icon={<TimerReset size={18} />} name="TimeJot" /><span class="route-arrow"><ArrowRight size={20} /></span><AppNode icon={<History size={18} />} name="Loop Habits" /></span>
        </button>
      </div>
    </section>
  );
